				...
				<div className="height-60p width-100 centered-vertical">
					<div className="width-100 padding-horizontal-5 overflow-hidden">
						<div className="flex-row overflow-hidden centered-vertical padding-5 radius-5 background-grey-hover pointer" onClick={() => history.push('/info')}>
							...
							<div className={`text-ellipsis text-700 text-18 ${location.pathname === "/info" ? 'text-pink' : 'text-grey'}`}>tims crazy zoo fun zone ya didg fame</div>
						</div> 
					</div>
				</div>
					...
					<div className="padding-horizontal-10">
						...
						{(authenticatedRoutes).map((data, i) => 
							<MenuOption
								...
								data={data}
								history={history}
								hideSideMenuAndNavigate={hideSideMenuAndNavigate}
								location={location}
							/>
						)}
					</div>
...
const mapDispatchToProps = dispatch => {
	return {
		hideSideMenuAndNavigate: (history, path) => dispatch(actionCreators.hideSideMenuAndNavigate(history, path)),
