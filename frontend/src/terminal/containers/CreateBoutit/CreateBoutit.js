...
import Overlay from '../../components/CreateBoutit/Overlay'
import Media from '../../components/CreateBoutit/Media'
import SearchBar from '../../components/Search/SearchBar'
import Title from '../../components/CreateBoutit/Title'
...
import {uploadMedia} from '../../../modules/s3'
...
class CreateBoutit extends Component{
	...
	handleChange = e => {
		...
		let keyType = typeof(this.state[name])
		switch(keyType){
			case 'string':
				this.setState({ [name]: value})	
				break
			case 'object':
				let newObj = Object.assign({}, this.state[name])
				newObj.value = value
	...
	getTypes = () => {
		let method = 'GET'
		return fetch('http://localhost:4001/types/types', {
			method,
		})
		.then(response => {
	...
	handleValidationError = errorObject => {
	...
	getMediaUrl = (buffer, bucket, contentType) => {
		uploadMedia(buffer, bucket, contentType)
			.then(result => {
				console.log(result)
			})
			.catch(error => {
	...
	sendType = () => {
		const key = 'typeText'
		const value = this.state.typeText.value
		const rules = this.state.typeText.validationRules
		let validationObj = checkValValidity(key, value, rules)
		if(validationObj.isValid){
			let type = {
				text: this.state.typeText.value
			}
			let method = 'POST'
			...
			.then(responseData => {
				this.setState({overlayVisible: false, typeText: ''})
			})
			.catch(error => {
			...
			this.handleValidationError(validationObj)
		}
	}
				...
				<div className="height-100 width-100flex-col relative">
					...
					{this.state.overlayVisible ?
						<Overlay
							...
							overlayCategory={this.state.overlayCategory}
